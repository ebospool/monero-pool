...
libevent and LMDB. Currently it uses only _two_ threads under normal operation
...
This pool was the _first_ pool to support RandomX and is currently the _only_
...
_own_ block template to mine on. Further information can be found in the
...
_polling_ for new blocks (using a timer), and instead fetch a new block template
when it receives a _signal_ (specifically, _SIGUSR1_). The Monero daemon,
`monerod`, has a feature whereby it can execute a command whenever a block is
...
_SIGUSR1_, to the pool whenever a new block is added to the chain.

Using this mechanism has a _significant_ benefit - your pool _immediately_ knows
...
As share validation is performed on the edge pools, it's **_vitally_** important
...
can of course all be configured to use the _same_ daemon, or for extra
...
config file. If you plan on running a _public_ pool via this UI (or any other
...
The web UI, as mentioned above, should ideally be placed behind a _caching
proxy_. Therefore SSL termination should be be configured there (i.e. in
Apache/Nginx).
...
This mining pool has **no built-in developer donation** (like _other_ mining
